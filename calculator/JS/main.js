...
const Calculator = {
    //this displays 0 on the sxcreen
    Display_Value:'0',
...
function Input_Digit(digit) {
    ...
    //we are checking to see if Wait_Second_opersand is true and set
    //display_value to the key that was clicked.
    if(Wait_Second_Operand === true) {
        Calculator.Display_Value = digit;
        Calculator.Wait_Second_Operand = false;
    } else {
        //this overwrites the display value if the current value is f0
...
function Input_Decimal (dot) {
    ...
    if(!Calculator.Display_Value.includes(dot)) {
        // we are saying that if the display value does not contain a decimal poin
...
function Handle_Operator(Next_Operator) {
    ...
    //checks if an operator already exists an if Wait_Second_Operand is true,
...
keys.addEventListener('click', (event) => {
    ...
    //ensures that AC clears the numbers fromt he Calculator
